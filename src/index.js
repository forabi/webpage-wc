...
const API_URL = 'https://www.readability.com';
...
const fetchPage = token => url => {
  ...
  return got(`${API_URL}/api/content/v1/parser`, {
...
if (!token) {
  try {
    ...
    process.stderr.write('Get your API token at https://www.readability.com/settings/account\n');
    process.exit(1);
  }
}
...
Promise.all(urls.map(fetchPage(token)))
  ...
  .catch(e => {
    if (e.response && e.response.body && e.response.body.messages) {
      ...
      if (!tokenIsNew && e.statusCode === 403) {
